...
const Pesquisa = () =>
{
  ...
  return (
    <div className='pt-6'>
            ...
            {notas.map(nota =>
            {
              return (
                <label key={'label'+nota} className='block w-1/6 text-center'>
                  {nota}
                  <br />
                  <input key={'imput'+nota} type='radio' name='Nota' value={nota} onChange={onChange} />
                </label>
              )
            })
            }
      ...
      {sucsess &&
        <div className='w-1/5 mx-auto'>
          <p className='mb-6 bg-blue-100 border-t border-b border-blue-500 text-blue-700 px-4 py-3'>Obrigado por contribuir com sua sugestão ou critica.</p>
          {
            retorno.showCupom &&
